...
from py_scripts.make_commit import make_commit
...
def test_make_get_request(mock_response):
    with patch('py_scripts.make_commit.make_commit.requests.get', return_value=mock_response) as mock_get:
...
def test_make_post_request(mock_response):
    with patch('py_scripts.make_commit.make_commit.requests.post', return_value=mock_response) as mock_post:
...
def test_get_latest_commit_sha():
    with patch('py_scripts.make_commit.make_commit.make_get_request') as mock_get_request:
...
def test_create_new_tree():
    with patch('py_scripts.make_commit.make_commit.make_post_request') as mock_post_request:
...
def test_update_ref(mock_response):
    with patch('py_scripts.make_commit.make_commit.requests.put', return_value=mock_response) as mock_put:
