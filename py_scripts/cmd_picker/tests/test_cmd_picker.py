...
from cmd_picker import TmuxTool, DockerTool, GhTool, CmdPicker
...
class TestDockerTool:
    ...
    def test_get_items_success(self, mock_run):
        tool = DockerTool()
        mock_run.return_value = Mock(
            stdout="abc123456789\ttest_container\tUp 2 hours\tnginx:latest\n",
            returncode=0
        )
...
class TestCmdPicker:
    ...
    @patch.object(TmuxTool, 'get_items', return_value=[
        {'name': 'session1', 'windows': '2', 'created': '1234567890', 'type': 'session'}
    ])
